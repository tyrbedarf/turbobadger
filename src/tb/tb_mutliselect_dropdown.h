...
namespace tb {
	...
	*/
	class TBMultiselectDropdown : public TBButton, public TBSelectItemViewer
	{
	public:
		// For safe typecasting
		TBOBJECT_SUBCLASS(TBMultiselectDropdown, TBButton);

		TBMultiselectDropdown();
		~TBMultiselectDropdown();
		...
		*/
		...
		TBPopupWindow *GetMenuIfOpen() const;
		...
		int GetNumItems() {
			return m_default_source.GetNumItems();
		}

		TBGenericStringItem* GetItem(int i) {
			return m_default_source.GetItem(i);
		}
