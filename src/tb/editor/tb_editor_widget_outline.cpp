...
namespace tb {
	...
	// == ResourceEditWindow ==============================================================================
	TBEditorWidgetOutline::TBEditorWidgetOutline()
		...
		, m_source_edit(nullptr)
	{
		...
		g_widgets_reader->LoadFile(this, "resources/editor_layout/tb_editor_widget_outline.tb.txt");
	...
	void TBEditorWidgetOutline::Load(const char *resource_file)
	{
	...
	void TBEditorWidgetOutline::RefreshFromSource()
	{
		// Clear old widgets
	...
	void TBEditorWidgetOutline::UpdateWidgetList(bool immediately)
	{
		if (!immediately)
		{
	...
	void TBEditorWidgetOutline::AddWidgetListItemsRecursive(TBWidget *widget, int depth)
	{
	...
	TBEditorWidgetOutline::ITEM_INFO TBEditorWidgetOutline::GetItemFromWidget(TBWidget *widget)
	{
	...
	void TBEditorWidgetOutline::SetSelectedWidget(TBWidget *widget)
	{
	...
	bool TBEditorWidgetOutline::OnEvent(const TBWidgetEvent &ev)
	{
	...
	void TBEditorWidgetOutline::OnPaintChildren(const PaintProps &paint_props)
	{
	...
	void TBEditorWidgetOutline::OnMessageReceived(TBMessage *msg)
	{
		if (msg->message == TBIDC("update_widget_list"))
			UpdateWidgetList(true);
	}

	bool TBEditorWidgetOutline::OnWidgetInvokeEvent(TBWidget *widget, const TBWidgetEvent &ev)
	{
	...
	void TBEditorWidgetOutline::OnWidgetAdded(TBWidget *parent, TBWidget *child)
	{
		if (m_build_container && m_build_container->IsAncestorOf(child))
			UpdateWidgetList(false);
	}

	void TBEditorWidgetOutline::OnWidgetRemove(TBWidget *parent, TBWidget *child)
	{
		if (m_build_container && m_build_container->IsAncestorOf(child))
			UpdateWidgetList(false);
	}

	bool TBEditorWidgetOutline::OnDropFileEvent(const TBWidgetEvent &ev)
	{
